...
Scenario: 1 learner aged 16-18, non-DAS. Second employment status record added with same employer id but small employer flag removed. Learner retains small employer funding.
    ...
    Then the provider earnings and payments break down as follows:
		...
		| Provider Earned from SFA                | 570   | 570   | 570   | 1570  | 570   | ... | 2710  | 0     |
		| Provider Earned from Employer           | 50    | 50    | 50    | 50    | 50    | ... | 150   | 0     |
		| Provider Paid by SFA                    | 0     | 570   | 570   | 570   | 1570  | ... | 570   | 2710  |
		| Payment due from Employer 98765432      | 0     | 0     | 0     | 0     | 0     | ... | 0     | 0     |
...
#DAS small employers
@SmallEmployerDas
 Scenario:AC6- Payment for a 16-18 DAS learner, small employer at start
...
@SmallEmployerDas   
 Scenario:AC7- Scenario: 1 learner aged 19-24, DAS, with an Education Health Care (EHC) plan, employed with a small employer at start, is fully funded for on programme and completion payments
    Given levy balance > agreed price for all months
    And the following commitments exist:
        | ULN       | framework code | programme type | pathway code | agreed price | start date |
        | learner a | 403            | 2              | 1            | 7500         | 06/08/2017 |
...
@SmallEmployerDas   
Scenario:AC8- 1 learner aged 19-24, DAS, is a care leaver, employed with a small employer at start, is fully funded for on programme and completion payments
    Given levy balance > agreed price for all months
    And the following commitments exist:
        | ULN       | framework code | programme type | pathway code | agreed price | start date |
        | learner a | 403            | 2              | 1            | 7500         | 06/08/2017 |
...
@SmallEmployerDas       
Scenario:AC9- 1 learner aged 19-24, DAS, employed with a small employer at start, is funded using levy for on programme and completion payments (this apprentice does not have a Education Health Care plan and is not a care leaver)
 ...
 Then the provider earnings and payments break down as follows:
  ...
  @SmallEmployerDas   
Scenario:AC10- Payment for a 16-18 DAS learner, employer is not small
...
Scenario:AC11- Payment for a 16-18 non-DAS learner, small employer at start, change to large employer
	When an ILR file is submitted with the following data:
...
@SmallEmployerMultipleEmploymentStatus
Scenario:AC12- Payment for a 16-18 non-DAS learner, large employer at start, change to small employer
	When an ILR file is submitted with the following data:
		| ULN       | learner type                 | agreed price | start date | planned end date | actual end date | completion status | framework code | programme type | pathway code | 
		| learner a | 16-18 programme only non-DAS | 7500         | 06/08/2017 | 08/08/2018       | 08/08/2018      | completed         | 403            | 2              | 1            | 
    And the employment status in the ILR is:
